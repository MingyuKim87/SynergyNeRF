...
class Trainer:
    ...
    def train(self):
        ...
        if self.cfg.model.plane_smooth_type == "TV":
            tvreg_s = TVLoss()  # TV loss on the spatial planes
            tvreg_s_t = TVLoss(
                1.0, self.cfg.model.TV_t_s_ratio
            )  # TV loss on the spatial-temporal planes
        elif self.cfg.model.plane_smooth_type == "LM":
            tvreg_s = LMLoss()  # TV loss on the spatial planes
            tvreg_s_t = LMLoss(
                1.0, self.cfg.model.TV_t_s_ratio
            )  # LM loss on the spatial-temporal planes
