...
class SynergyNeRF_Base(torch.nn.Module):
    ...
    def init_density_func(self, density_mode, t_pe, pos_pe, feature_dim, n_layers, device):
        ...
        if (density_mode == "plain"):  # Use extracted features directly from SynergyNeRF as density.
            ...
            self.density_regressor = Grid_Coord_Mixing_MLP(
                ...
                t_pe=t_pe, pos_pe=pos_pe, feature_dim=feature_dim, n_layers=n_layers, zero_init=True).to(device)
        else:
            raise NotImplementedError("Invalid Density Regression Mode")
        
        print("[Model] Density regressor:")
        print(self.density_regressor)
        print("*"*10, f"t_pe : {t_pe} pos_pe : {pos_pe}", "*"*10)

    def init_app_func(self, app_mode, t_pe, pos_pe, view_pe, fea_pe, feature_dim, n_layers, device):
        ...
        if app_mode == "SH":  # Use Spherical Harmonics SH to render appearance.
            ...
            self.app_regressor = General_MLP(SH_feature_dim+self.app_dim, 3, 
                                             t_pe, fea_pe, pos_pe, view_pe, 
                                             feature_dim, n_layers, 
                                             use_sigmoid=True, zero_init=True).to(device)
        else:
            raise NotImplementedError("Invalid App Regression Mode")
        
        print("[Model] App regressor:")
        print(self.app_regressor)
        print("*"*10, f"t_pe : {t_pe} pos_pe : {pos_pe} view_pe : {view_pe} fea_pe : {fea_pe}", "*"*10)
    ...
    def compute_tensorfeature(self, xyz_sampled, frame_time, progress):
        pass
    ...
    def filtering_rays(
        ...
        print(f"[Filtering all rays in traning data] The ratio of dropped ray : {torch.sum(mask_filtered) / N}")
    ...
    def forward(
        ...
        app_features = torch.zeros((*xyz_sampled.shape[:2], self.app_dim), device=xyz_sampled.device)

        # Compute density feature and density if there are valid rays.
        if ray_valid.any():
            density_feature = self.compute_tensorfeature(xyz_sampled[ray_valid], frame_time[ray_valid], progress)
            density_feature = self.density_regressor(
                xyz_sampled[ray_valid], viewdirs[ray_valid], density_feature, frame_time[ray_valid]
            )
        ...
        SH_features = SH_featrue(
                xyz_sampled=None, viewdirs=viewdirs, features=None, time=None
            )
        ...
        if app_mask.any():
            ...
            valid_rgbs = self.app_regressor(
                xyz_sampled[app_mask], viewdirs[app_mask], app_features[app_mask], frame_time[app_mask],
            )
    ...
    def forward_no_tensor_feature(
        ...
        # Compute density feature and density if there are valid rays.
        if ray_valid.any():
            Valid_xyz_sampled = xyz_sampled[ray_valid]; 

            # dummy density_feature
            density_feature = torch.zeros((*Valid_xyz_sampled.shape[:-1], int(sum(self.density_n_comp))), device=xyz_sampled.device)
            density_feature = self.density_regressor(
                xyz_sampled[ray_valid], viewdirs[ray_valid], density_feature, frame_time[ray_valid]
            )
    ...
    @torch.no_grad()
    def update_alpha_mask(self, grid_size=(200, 200, 200), time_grid=64, progress=1.0):
        """
        ...
        alpha_grid, dense_xyz = self.get_alpha_grid(grid_size, time_grid, progress)
    ...
    @torch.no_grad()
    def get_alpha_grid(self, grid_size=None, time_grid=None, progress=1.0):
        """
        ...
        for i in range(grid_size[0]):
            alpha_grid[i] = self.compute_alpha_for_alpha_grid(
                dense_xyz[i].view(-1, 3).contiguous(), time_grid, self.step_size, progress
            ).view((grid_size[1], grid_size[2]))
        return alpha_grid, dense_xyz

    def compute_alpha_for_alpha_grid(self, xyz_locs, time_grid=64, length=1, progress=1.0):
        """
        Compute the emptiness of spacetime points. Emptiness is the density.
        For each space point, we calcualte its densitis for various time steps and calculate its maximum density.
        """
        if self.alpha_mask is not None:
        ...
        if density_mask.any():
            ...
            density_feature = self.compute_tensorfeature(xyz_sampled, time_samples, progress)
            density_feature = self.density_regressor(
                xyz_sampled,
                xyz_sampled,
                density_feature,
            ...
            sigma_feature = sigma_feature.view(N, T)
    ...
    def get_tensorial_features(self):
        density_plane_clone_list = []; density_line_time_clone_list = []

        for i in range(len(self.density_plane)):
            density_plane_comp = np.squeeze(self.density_plane[i].clone().detach().cpu().numpy())
            density_line_comp = np.squeeze(self.density_line_time[i].clone().detach().cpu().numpy())

            density_plane_clone_list.append(density_plane_comp)
            density_line_time_clone_list.append(density_line_comp)
        
        
        return density_plane_clone_list, density_line_time_clone_list