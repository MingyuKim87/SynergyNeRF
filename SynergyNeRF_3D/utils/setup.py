...
def set_config():
    ...
    # Default config
    if ("TensorVMSplit" in yaml_cfg.model.model_name) or ("TensorCP" in yaml_cfg.model.model_name) \
        or ("TensorVM" in yaml_cfg.model.model_name):
        from config.TensoRF.config import Config
        ...
        if not len(yaml_cfg.model.model_name.split("_")) > 1:
            ...
            if "disentangled" in yaml_cfg.model.model_name:
    ...
    # curriculum learning
    if ("SynergyNeRF" in yaml_cfg.model.model_name) and (cfg.model.curriculum_start is not None):
...
def set_logdir(cfg):
    ...
    render_mesh_path = f"{render_root_path}/mesh"
    ...
    if cfg.render_disentangled: os.makedirs(render_disentangled_path, exist_ok=True)
    if cfg.render_mesh: os.makedirs(render_mesh_path, exist_ok=True)
    ...
    return {
        ...
        "logdir_render_mesh" : render_mesh_path if cfg.render_mesh else None,
