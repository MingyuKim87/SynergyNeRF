...
class TCNN_General_MLP(torch.nn.Module):
    """
    A general MLP module with potential input including time position encoding(PE): feature PE: fea_pe, 3D position PE: pos_pe,
    view direction PE: view_pe.
    ...
    def forward(
        ...
        rgb = self.mlp(mlp_in)
        if not self.fp16 : rgb = rgb.float()
...
class TCNN_Grid_Coord_Mixing_MLP(torch.nn.Module):
    """
    A general MLP module with potential input including time position encoding(PE): feature PE: fea_pe, 3D position PE: pos_pe,
    view direction PE: view_pe.
