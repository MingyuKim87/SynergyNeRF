...
class SynergyNeRF_Base(torch.nn.Module):
    ...
    def init_density_func(self, density_mode, pos_pe, feature_dim, n_layers, device):
        ...
        if (density_mode == "plain"):  # Use extracted features directly from SynergyNeRF as density.
            ...
            self.density_regressor = Grid_Coord_Mixing_MLP(
                ...
                pos_pe=pos_pe, feature_dim=feature_dim, n_layers=n_layers, zero_init=True).to(device)
        else:
    ...
    def forward(
        ...
        n_samples: int = -1,
        progress: float = 1.0, 
        is_test_disentangled: bool = False
    ...
    def forward_no_tensor_feature(
        ...
        """
        Forward pass of the SynergyNeRFPlane.
    ...
    def set_attributes(self, base):
        for base_attr, base_value in vars(base).items():
            if base_attr in vars(self):
                setattr(self, base_attr, base_value)
