...
class TensorVM(TensorBase):
    def __init__(self, aabb, gridSize, near_far, device, **kargs):
        super(TensorVM, self).__init__(aabb, gridSize, near_far, device, **kargs)
        
    def init_svd_volume(self, res, device):
        self.plane_coef = torch.nn.Parameter(
            self.init_shift + self.init_scale * torch.randn((3, self.app_n_comp + self.density_n_comp, res, res), device=device))
        self.line_coef = torch.nn.Parameter(
            self.init_shift + self.init_scale * torch.randn((3, self.app_n_comp + self.density_n_comp, res, 1), device=device))
        self.basis_mat = torch.nn.Linear(self.app_n_comp * 3, self.app_dim, bias=False, device=device)

    def get_optparam_groups(self, lr_init_spatialxyz = 0.02, lr_init_network = 0.001, **kwargs):
...
class TensorVMSplit(TensorBase):
    def __init__(self, aabb, gridSize, near_far, device, **kargs):
        super(TensorVMSplit, self).__init__(aabb, gridSize, near_far, device, **kargs)


    def init_svd_volume(self, res, device):
        self.density_plane, self.density_line = self.init_one_svd(self.density_n_comp, self.gridSize, device)
        self.app_plane, self.app_line = self.init_one_svd(self.app_n_comp, self.gridSize, device)
    ...
    def init_one_svd(self, n_component, gridSize, device):
        ...
        for i in range(len(self.vecMode)):
            ...
            plane_coef.append(torch.nn.Parameter(
                self.init_shift + self.init_scale * torch.randn((1, n_component[i], gridSize[mat_id_1], gridSize[mat_id_0]))))  #
            line_coef.append(
                torch.nn.Parameter(self.init_shift + self.init_scale * torch.randn((1, n_component[i], gridSize[vec_id], 1))))
    ...
    def get_num_params(self):
        ...
        for p in grad_vars:
            if isinstance(p["params"], torch.nn.ParameterList):
                for pp in p["params"]:
                    total_params += pp.numel()
            else:
                total_params += sum(pr.numel() for pr in p["params"])
    ...
    def TV_loss_density(self, reg):
        total = 0
        for idx in range(len(self.density_plane)):
            total = total + reg(self.density_plane[idx]) #+ reg(self.density_line[idx]) * 1e-3
        return total
        
    def TV_loss_app(self, reg):
        total = 0
        for idx in range(len(self.app_plane)):
            total = total + reg(self.app_plane[idx]) #+ reg(self.app_line[idx]) * 1e-3
        return total
...
class TensorCP(TensorBase):
    def __init__(self, aabb, gridSize, near_far, device, **kargs):
        super(TensorCP, self).__init__(aabb, gridSize, near_far, device, **kargs)


    def init_svd_volume(self, res, device):
        self.density_line = self.init_one_svd(self.density_n_comp[0], self.gridSize, device)
        self.app_line = self.init_one_svd(self.app_n_comp[0], self.gridSize, device)
    ...
    def init_one_svd(self, n_component, gridSize, device):
        ...
        for i in range(len(self.vecMode)):
            vec_id = self.vecMode[i]
            line_coef.append(
                torch.nn.Parameter(self.init_shift + self.init_scale * torch.randn((1, n_component, gridSize[vec_id], 1))))
